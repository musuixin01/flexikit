...
@Injectable()
export class OschinaCrawlerService extends CrawlerService {
  private readonly feedUrl = 'https://www.oschina.net/news/rss';
  private readonly source = 'oschina';
  ...
  async crawl(): Promise<number> {
    ...
    try {
      ...
      this.logger.log(`开源中国抓取完成，新增 ${count} 个工具`);
      return count;
    } catch (e) {
      this.logger.error('开源中国抓取失败', e);
      return 0;
    }
  }
  ...
  private async fetchFeed(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      req.on('timeout', () => {
        req.destroy();
        reject(new Error('开源中国 RSS 请求超时'));
      });

      req.on('error', (e) => {
        this.logger.error('开源中国 RSS 请求失败', e);
        reject(e);
      });
    });
  }
  ...
  private extractUrl(html: string): string {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      if (href && !href.includes('oschina.net') && !href.includes('#')) {
        // 只保留 http/https 链接
  ...
  private classifyCategory(text: string): string {
    ...
    if (/(开源|GitHub|开源项目|开源软件)/i.test(text)) {
      return '开发工具';
    }

    return '开发工具'; // 默认分类（开源中国主要是开发相关）
  }
