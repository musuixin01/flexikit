...
@Injectable()
export class IplaysoftCrawlerService extends CrawlerService {
  private readonly feedUrl = 'https://www.iplaysoft.com/feed/';
  private readonly source = 'iplaysoft';
  ...
  async handleCron() {
    this.logger.log('定时任务：开始抓取异次元软件');
    await this.crawl();
  }
  ...
  async crawl(): Promise<number> {
    ...
    try {
      ...
      this.logger.log(`异次元软件抓取完成，新增 ${count} 个工具`);
      return count;
    } catch (e) {
      this.logger.error('异次元软件抓取失败', e);
      return 0;
    }
  }
  ...
  private async fetchFeed(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      req.on('timeout', () => {
        req.destroy();
        reject(new Error('异次元软件 RSS 请求超时'));
      });

      req.on('error', (e) => {
        this.logger.error('异次元软件 RSS 请求失败', e);
        reject(e);
      });
    });
  }
  ...
  private extractUrl(html: string): string {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      if (href && !href.includes('iplaysoft.com') && !href.includes('#')) {
        // 只保留 http/https 链接
  ...
  private classifyCategory(text: string): string {
    ...
    if (/(视频|音频|播放|剪辑|录屏|音乐|播放器|媒体)/i.test(text)) {
      return '影音娱乐';
    }
    if (/(图片|图像|截图|编辑|处理|设计|PS|Photoshop|修图|抠图)/i.test(text)) {
      return '图形图像';
    }
    if (/(开发|编程|代码|IDE|调试|Git|API|程序员|开发工具)/i.test(text)) {
      return '开发工具';
    }
    if (/(系统|优化|清理|监控|工具|软件|Windows|macOS)/i.test(text) && /(系统|优化|清理|监控|卸载|安装)/i.test(text)) {
      return '系统工具';
    }
    if (/(网络|下载|上传|加速|代理|浏览器|网盘|云盘)/i.test(text)) {
      return '网络工具';
    }
    if (/(安全|加密|密码|隐私|防护|杀毒|防毒)/i.test(text)) {
      return '安全工具';
    }
    if (/(文档|办公|PDF|笔记|写作|表格|PPT|Office)/i.test(text)) {
      return '文档办公';
    }
    if (/(效率|办公|时间管理|生产力|自动化|启动器|快捷键)/i.test(text)) {
      return '效率工具';
    }
    if (/(学习|教育|阅读|知识|语言|英语|单词|背单词)/i.test(text)) {
      return '学习教育';
    }
    if (/(文件|管理|云存储|同步|备份|压缩|解压|文件管理器)/i.test(text)) {
      return '文件管理';
    }
    if (/(手机|移动|安卓|Android|iOS|APP|应用)/i.test(text)) {
      return '手机工具';
    }
    if (/(游戏|娱乐|电竞|休闲|模拟器)/i.test(text)) {
      return '游戏娱乐';
    }
    if (/(设计|UI|UX|原型|矢量|插画|3D|建模)/i.test(text)) {
      return '设计工具';
    }
  ...
  private generateTags(text: string, category: string): string[] {
    ...
    // 添加一些关键词标签
    if (/(免费|开源|免费软件|免费版)/i.test(text)) {
      tags.push('免费');
    }
    if (/(开源|open source|GitHub|开源软件)/i.test(text)) {
      tags.push('开源');
    }
    if (/(便携|绿色|Portable|绿色版)/i.test(text)) {
      tags.push('便携软件');
    }
    if (/(Windows|win|Win)/i.test(text)) {
      tags.push('Windows');
    }
    if (/(macOS|Mac|苹果|Mac OS)/i.test(text)) {
      tags.push('macOS');
    }
    if (/(Linux|ubuntu|Linux软件)/i.test(text)) {
      tags.push('Linux');
    }
    if (/(中文|汉化|中文版)/i.test(text)) {
      tags.push('中文');
    }
    if (/(破解|注册|激活|补丁|绿色版|破解版)/i.test(text)) {
      // 不添加破解相关标签
    }
  ...
  private calculateHotScore(pubDate: string, contentLength: number): number {
    let score = 45; // 基础分（略低于小众软件，因为小众软件质量更高）
