...
@Injectable()
export class IfanrCrawlerService extends CrawlerService {
  private readonly feedUrl = 'https://www.ifanr.com/feed';
  private readonly source = 'ifanr';
  ...
  async crawl(): Promise<number> {
    ...
    try {
      ...
      this.logger.log(`爱范儿抓取完成，新增 ${count} 个工具`);
      return count;
    } catch (e) {
      this.logger.error('爱范儿抓取失败', e);
      return 0;
    }
  }
  ...
  private async fetchFeed(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      req.on('timeout', () => {
        req.destroy();
        reject(new Error('爱范儿 RSS 请求超时'));
      });

      req.on('error', (e) => {
        this.logger.error('爱范儿 RSS 请求失败', e);
        reject(e);
      });
    });
  }
  ...
  private extractUrl(html: string): string {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      if (href && !href.includes('ifanr.com') && !href.includes('#')) {
        // 只保留 http/https 链接
